...
void loop() {
  ...
  if (!robot.isBusy()) {
    switch (robot.getState()) {
      ...
      // After wave hello, move back to forward
      case WAVE_HELLO:
        Serial.println("Wave hello complete, moving back to forward");
 ...
 *     standby(), ready(), forward(), backward(), turnLeft(), turnRight(), moveLeft(), moveRight(), waveHello(), idle(), update()
 * 
 * - Movement_Driver.cpp
 ...
 *   STANDBY → READY → FORWARD → TURN_LEFT → TURN_RIGHT → BACKWARD → MOVE_LEFT → MOVE_RIGHT → WAVE_HELLO → FORWARD with IDLE periods
 * 
 * - How/Why?:
