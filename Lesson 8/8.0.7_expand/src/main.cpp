 ...
 *   - Then follow sequence:
       forward twice, turn left, turn right, backward twice, move left, move right, wave hello,
       perfom dance routines, lie down, fighting, push ups, sleep, loop back to forward.
 * 
...
void loop() {
  ...
  if (!robot.isBusy()) {
    switch (robot.getState()) {
      ...
      // After dance routine 3, move to lie down
      case DANCE3:
        Serial.println("Dance routine 3 complete, moving to lie down");
        robot.idle(idleTime, LIE_DOWN);    // lie down after idle time
        break;

      // After lie down, move to fighting
      case LIE_DOWN:
        Serial.println("Lie down complete, moving to fighting");
        robot.idle(idleTime, FIGHTING);    // fighting after idle time
        break;

      // After fighting, move to push ups
      case FIGHTING:
        Serial.println("Fighting complete, moving to push ups");
        robot.idle(idleTime, PUSH_UPS);    // push ups after idle time
        break;

      // After push ups, move to sleep
      case PUSH_UPS:
        Serial.println("Push ups complete, moving to sleep");
        robot.idle(idleTime, SLEEP);    // sleep after idle time
        break;

      // After sleep, move back to forward
      case SLEEP:
        Serial.println("Sleep complete, looping back to forward");
        robot.forward();
        break;
 ...
 *     standby(), ready(), forward(), backward(), turnLeft(), turnRight(), moveLeft(), moveRight(), waveHello(),
 *     dance1(), dance2(), dance3(), lieDown(), fighting(), pushUps(), sleep(), idle(), begin(), update()
 * 
 * - Movement_Driver.cpp
 ...
 *   STANDBY → READY → FORWARD → TURN_LEFT → TURN_RIGHT → BACKWARD → MOVE_LEFT → MOVE_RIGHT → WAVE_HELLO →
 *   DANCE ROUTINES → LIE_DOWN → FIGHTING → PUSH_UPS → SLEEP → FORWARD with IDLE periods
 * 
 * - How/Why?:
