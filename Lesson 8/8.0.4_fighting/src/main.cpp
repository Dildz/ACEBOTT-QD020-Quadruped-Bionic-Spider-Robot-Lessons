/*
 * This function is used to make the robot move into fighting positions.
 * 
 * NOTES:
...
const int fightArray[15][9] = {   // there are 15 arrays with 9 columns (adjust the MS value to change the speed)
// URP---URA---LRA---LRP---ULP---ULA---LLA---LLP---MS
...
// MAIN LOOP
void loop() {
  fighting();    // call center position function
  delay(1000);
}
 ...
 * 
 * FUNCTIONS:
 * - fighting() function uses the position arrays to move the limbs
 ...
 * - loop() function calls the fighting() function which runs repeatedly as long as the board is powered on.
 */
