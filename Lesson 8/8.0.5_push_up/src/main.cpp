/*
 * This function is used to make the robot do push ups.
 * 
 * NOTES:
...
// Define a push up movement array (modified dance3Array, starting from ready values)
const int pushupArray[21][9] = {   // there are 21 arrays with 9 columns (adjust the MS value to change the speed)
// URP---URA---LRA---LRP---ULP---ULA---LLA---LLP---MS
...
void pushUp() {    // Push up movement positions function
  writeServoPositions(pushupArray, 21);
}
...
// MAIN LOOP
void loop() {
  pushUp();    // call center position function
  delay(1000);
}
 ...
 * - pushupArray has 21 rows and 9 columns
 ...
 * 
 * FUNCTIONS:
 * - pushUp() function uses the position arrays to move the limbs
 ...
 * - loop() function calls the pushUp() function which runs repeatedly as long as the board is powered on.
 */