 ...
 * 
 * Modifications made:
 * - renamed DEFAULT mood to EYES_DEFAULT for better clarity and to avoid potential conflicts with Arduino's DEFAULT macro
 * - restructured Eyes Geometry section for better organization
 * - changed type `int` frameInterval to `unsigned long` to avoid potential overflow issues
 */
...
unsigned long frameInterval = 20; // default value for 50 frames per second (1000/50 = 20 milliseconds)
...
// STATE VARIABLES (Depends on defaults)
// Left eye
...
// COORDINATES (Initialized in begin() to avoid warnings/garbage values)
...
void begin(int width, int height, byte frameRate) {
  ...
  // INITIALIZE COORDINATE MATH HERE
  // This ensures screenWidth/Height are set before calculations
