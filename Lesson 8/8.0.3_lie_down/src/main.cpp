/*
 * This function is used to make the robot move into a lying down position.
 * 
 * NOTES:
...
// Define a lying down position array
const int lieDownArray[1][9] = {    // 1 arrays with 9 columns (0 to 8)
// URP---URA---LRA---LRP---ULP---ULA---LLA---LLP---MS
  {110,   90,   90,   70,   70,   90,   90,  110,  500},    // lift legs up
};
...
void lieDown() {    // Lie down position function
  writeServoPositions(lieDownArray, 1);
}
...
// MAIN LOOP
void loop() {
  lieDown();    // call center position function
}
 ...
 * 
 * ARRAY:
 * - standbyArray and lieDownArray have 1 row and 9 columns
 ...
 * 
 * FUNCTIONS:
 * - lieDown() function uses the position array to move the limbs
 * - setup() function runs once when the program starts and attaches the servos, writes the positions in the standbyArray.
 * - loop() function calls the lieDown() function which runs repeatedly as long as the board is powered on.
 */