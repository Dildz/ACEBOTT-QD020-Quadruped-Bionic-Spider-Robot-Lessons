/*
 * This function is used to turn the robot right.
 * 
 * NOTES:
...
const int turnRightArray[9][9] = {    // there are 9 arrays with 9 columns (adjust the MS value to change the speed)
// URP---URA---LRA---LRP---ULP---ULA---LLA---LLP---MS
...
void right() {    // Right function
  writeServoPositions(turnRightArray, 9);    // move through all 9 steps
}
...
// MAIN LOOP
void loop() {
  right();    // call the right turn function
  delay(1000);
}
 ...
 * - turnRightArray has 9 rows and 9 columns
 ...
 * 
 * FUNCTIONS:
 * - right() function uses the position array to move the limbs
 ...
 * - loop() function calls the right() function with a 1 second delay which runs repeatedly as long as the board is powered on.
 */