 ...
 *   ( robot.standby(), (robot.ready(), robot.forward(), (robot.backward(), (robot.turnLeft(), (robot.turnRight() ).
...
void loop() {
  ...
  if (!robot.isBusy()) {
    switch (robot.getState()) {
      ...
      // After backward, check if we need to do it again or return to forward
      case BACKWARD:
        backwardCount++;
        if (backwardCount < repeatAmount) {
          ...
          Serial.println("Backward complete, returning to forward");
          backwardCount = 0;
          robot.idle(idleTime, FORWARD);    // move forward after idle time
        }
        break;
 ...
 *   - Provides the public API: standby(), ready(), forward(), backward(), turnLeft(), turnRight(), idle(), update()
 * 
 * - Movement_Driver.cpp
 ...
 * - The state machine automatically advances: STANDBY → READY → FORWARD → TURN_LEFT → TURN_RIGHT → BACKWARD → FORWARD with IDLE periods
 * 
 * - How/Why?:
